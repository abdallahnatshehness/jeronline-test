<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	<xsl:output method="html"/>
		<xsl:template match="/*">
			<html>
				...
				<head>
					<xsl:apply-templates select="Headers/Header[@type = 'fail']"/>
				</head>
				<body>
	...
	<xsl:template match="Table">
		<xsl:if test="count(Rows/Row[@type = 'fail']) != 0">
			<table>
				...
				<xsl:apply-templates select="Rows/Row[@type = 'fail']"/>
			</table>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Row[@type = 'fail']">
	<tr>
